...
public class SuperLevelOptimizerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        else if (slo.searchState == SearchState.ByTag)
        {
            slo.byTag = EditorGUILayout.TextField("Tag : ", slo.byTag);
    ...
    private Renderer[] GetRenderers()
    {
        ...
        if (slo.searchState == SearchState.ByTag)
            return (from g in GameObject.FindGameObjectsWithTag(slo.byTag)
                    where g.GetComponent<Renderer>() != null
                    where GameObjectUtility.AreStaticEditorFlagsSet(g, StaticEditorFlags.BatchingStatic)
                    where g.GetComponent<MeshFilter>() != null
                    where g.GetComponent<MeshFilter>().sharedMesh != null
                    select g.GetComponent<Renderer>()).ToArray();
